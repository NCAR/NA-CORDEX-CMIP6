...
# Default location for the data request CSV (used to get variable metadata)
...
# The python one-liner is used here to handle YAML anchors/aliases.
get_spec() {
    local var="$1" field="$2"
    python3 -c "
import yaml
...
# Load the data request CSV to get CMOR metadata per variable.
# INDIR contains the CSV produced (or copied) by setup.sh.
...
for vardir in "$INDIR"/*/; do
    ...
    for infile in "${files[@]}"; do
        ...
        # cmorize.sh positional arguments match the original cmorize.compress.sh interface:
        # wrfout_path var fname freq units lev refh cell ln stdn year outdir

        # year is extracted from the filename timespan field.
        year="${fname##*_}"   # last underscore-delimited field
        year="${year:0:4}"    # first 4 characters = start year

        echo "peak_memusage time csh -t './cmorize.sh $INDIR $varname $infile $freq \"$units\" $levels $refh \"$cell\" \"$ln\" \"$stdn\" $year $OUTDIR'" >> "$cmdfile"
        (( ncommands++ )) || true
    done
