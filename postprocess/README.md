...
6. Run `aggregate.sh $indir $outdir $version $cmddir` where:
   ...
   * `$cmddir` is the directory where you want the commandfiles
   ...
   The version number should look like "vYYYYMMDD", where Y/M/D is the
   year/month/day when the simulation completed.  You can find that
   date by running this command in the directory with the raw output:
