# NA-CORDEX-CMIP6 Postprocessing Workflow (bash)
...
All shell syntax below is written for bash.
...
pad() { awk '{ for (i=1; i<=NF; i++) printf "%10s", $i; print ""}'; }

cd $rundir1
echo -e "\t\t    max memory\t\t      runtime"
echo -e "var\tDone:\tmin\tmean\tmax\tmin\tmean\tmax\tother final lines"
for i in *; do
  echo -ne "$i\t"
...
indir7drs=$(find $topdir/CORDEX-CMIP6 -type d -name v1-r1)
...
for f in esgqa/stdout*/*; do tac "$f" | sed "/###/q" | tac; done | cut -f1 -d: | sort | uniq -c
